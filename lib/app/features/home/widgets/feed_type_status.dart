...
import 'package:audio_cult/app/features/home/widgets/feed_type_album.dart';
...
class _FeedTypeStatusState extends State<FeedTypeStatus> {
  ...
  Widget _buildContent(FeedResponse data, BuildContext context) {
    ...
    if (data.parentModuleId != null) {
      if (data.parentModuleId == 'music_song') {
        return FeedTypeSong(
          parentFeed: data.parentFeed,
        );
      } else {
        return FeedTypeAlbum(
          feedStatus: data.feedStatus,
          customDataCache: data.parentFeed?.customDataCache,
        );
      }
    } else {
