...
import '../features/audio_player/audio_player.dart';
import '../features/audio_player/queue_state.dart';
...
class AudioPlayerHandlerImpl extends BaseAudioHandler with QueueHandler, SeekHandler implements AudioPlayerHandler {
  ...
  Future<void> _init() async {
    ...
    mediaItem.whereType<MediaItem>().listen((item) {
      ...
      if (item.artUri.toString().startsWith('http') && item.genre != 'YouTube') {
    ...
    _effectiveSequence
        .map(
          (sequence) => sequence.map((source) => _mediaItemExpando[source]!).toList(),
        )
        .pipe(queue);

    if (loadStart) {
      final List lastQueueList = await Hive.box('cache').get('lastQueue', defaultValue: [])?.toList() as List;

      final int lastIndex = await Hive.box('cache').get('lastIndex', defaultValue: 0) as int;

      final int lastPos = await Hive.box('cache').get('lastPos', defaultValue: 0) as int;

      final List<MediaItem> lastQueue = lastQueueList.map((e) => MediaItemConverter.mapToMediaItem(e as Map)).toList();
  ...
  AudioSource _itemToSource(MediaItem mediaItem) {
    ...
    if (mediaItem.artUri.toString().startsWith('file:')) {
      _audioSource = AudioSource.uri(Uri.file(mediaItem.extras!['url'].toString()));
    } else {
  ...
  Future<void> startService() async {
    final bool withPipeline = Hive.box('settings').get('supportEq', defaultValue: false) as bool;
  ...
  Future<void> addLastQueue(List<MediaItem> queue) async {
    final lastQueue = queue.map((item) => MediaItemConverter.mediaItemtoMap(item)).toList();
  ...
  Future customAction(String name, [Map<String, dynamic>? extras]) {
    ...
    if (name == 'sleepCounter') {
      if (extras?['count'] != null && extras!['count'].runtimeType == int && extras['count'] > 0 as bool) {
  ...
  Future<Map> getEqParms() async {
    ...
    final List<Map> bandList = bands
        .map(
          (e) => {'centerFrequency': e.centerFrequency, 'gain': e.gain, 'index': e.index},
        )
        .toList();
