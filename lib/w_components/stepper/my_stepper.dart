...
class _StepperState extends State<Stepper> with TickerProviderStateMixin {
  ...
  Color _circleColor(int index) {
    ...
    if (!_isDark()) {
      return widget.steps[index].isActive ? colorScheme.primary : Colors.white;
    } else {
      ...
      for (int i = 0; i < widget.steps.length; i += 1) ...<Widget>[
        ...
        if (!_isLast(i))
          Expanded(
            child: Container(
              height: 4.0,
              color: widget.steps[i].state == StepState.complete ? AppColors.activeLabelItem : Colors.grey.shade400,
            ),
          ),
      ],
    ];
